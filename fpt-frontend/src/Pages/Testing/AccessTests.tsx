...
export default function AccessTests()  {
    ...
    const checkUserRole = async () => {
        try {
            const response = await axios.get("https://localhost:7206/UserAccount/testUserRole", {withCredentials: true});
    ...
    const checkSuperUserRole = async () => {
        try {
            const response = await axios.get(baseUrl+"/testSuperUserRole", {withCredentials: true});
    ...
    const checkCookie = async () => {
        try {
            const response = await axios.get(baseUrl+"/checkcookie", { withCredentials: true });
            setMessage(
