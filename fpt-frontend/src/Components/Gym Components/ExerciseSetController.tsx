...
export function ExerciseSetController({
  ...
  const updateExerciseSet = (updater: (prev: Set) => Set) => {
    updateExerciseSetBloc?.((prevExerciseSetBloc) => ({
      ...prevExerciseSetBloc,
      sets: prevExerciseSetBloc.sets.map((set) =>
        (set.id ?? set.tempId) === (exerciseSet.id ?? exerciseSet.tempId)
          ? updater(set)
          : set,
      ),
    }));
  ...
  const updateSetValues = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    updateExerciseSet((prev) => ({
      ...prev,
      [name]:
        name === "repFloor" || name === "repCeiling" ? Number(value) : value,
    }));
  };
  ...
  return (
    <div>
      ...
      <input
        ...
        type="number"
      />
      <label htmlFor={"repCeiling"}>Rep Ceiling</label>
      <input
        name={"repCeiling"}
        onChange={updateSetValues}
      ...
      <input
        ...
        type="text"
      />
      <button onClick={removeExerciseSet}> Remove Set</button>
    </div>
  );
}
