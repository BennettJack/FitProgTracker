...
{
    public MuscleService(FptDbContext context, ICurrentUserService currentUserService)
    ...
    public override async Task<List<DropdownReturnDto>> GetListAsDropdownAsync()
    {
        ...
        var dropdownList = equipment
            .Select(m => new DropdownReturnDto { Value = m.Id, Label = m.MuscleName })
            .ToList();
